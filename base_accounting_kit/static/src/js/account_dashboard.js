    ...
    var ActionMenu = AbstractAction.extend({
        ...
        events: {
            ...
            'change #start_date': function(e) {
                            ...
                            this.onclick_start_date({'analytic_account': this.$('#analytic_account').val(),'start_date': this.$('#start_date').val(), 'end_date': this.$('#end_date').val()});
                                                        },
            ...
            'change #analytic_account': function(e) {
                            e.stopPropagation();
                            var $target = $(e.target);
                            var value = $target.selectedIndex;
                            this.onclick_start_date({'analytic_account': this.$('#analytic_account').val(),'start_date': this.$('#start_date').val(), 'end_date': this.$('#end_date').val()});
                                                        },
            'change #balance_date': function(e) {
                            e.stopPropagation();
                            var $target = $(e.target);
                            var value = $target.val();
                            this.onclick_balance_date({'balance_date': this.$('#balance_date').val()});
                                                        },
...
//                            this.$('.aged_receivable_this_month').empty();
        ...
        onclick_balance_date: function (f) {
            var self = this;
            var selected = $('.btn.btn-tool.income');
            var data = $(selected[0]).data();
            var posted = false;
        ...
        renderElement: function (ev) {
            ...
            $.when(this._super())
                .then(function (ev) {
                    ...
                    rpc.query({
                        ...
                        .then(function (result) {
                            _.forEach(result, function (value, key) {
                                $('#analytic_account').append($('<option>', { value : value['id'] }).text(value['name'])); 
                            })
                        })

                    rpc.query({
                        model: "account.move",
                        method: "get_date_values",
                    })
                        .then(function (result) {
                            $('#start_date').attr("value", result['start_date'])
                            $('#end_date').attr("value", result['end_date'])

                            var arg = {'start_date': result['start_date'], 'end_date': result['end_date'], 'analytic_account': false}
                            rpc.query({
                                model: "account.move",
                                method: "get_income_start_date",
                                args: [posted,arg],
                    ...
                    rpc.query({
                        model: "account.move",
                        method: "get_balance_date_value",
                    })
                        .then(function (result) {
                            $('#balance_date').attr("value", result['balance_date'])

                            var arg = {'balance_date': result['balance_date']}
