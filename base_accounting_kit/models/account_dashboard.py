...
class DashBoard(models.Model):
    ...
    # function to getting income by start date

    @api.model
    def get_income_start_date(self, *post):
        company_id = self.get_current_company_value()
        analytic_account = False
        if len(post) > 1 and post[1] and post[1]['analytic_account']:
            analytic_account = post[1]['analytic_account']
        start_date = datetime.strptime(post[1]['start_date'], '%Y-%m-%d')
        start_month = start_date.month
        start_date = start_date.strftime(DEFAULT_SERVER_DATE_FORMAT)
        end_date = datetime.strptime(post[1]['end_date'], '%Y-%m-%d')
    ...
    # function to get analytic account

    @api.model
    def get_field_values(self):
        values = self.env['account.analytic.account'].search([])
        datalist = []
        for val in values:
            datalist.append({'id': val.id, 'name': val.name})
        return datalist
    ...
    @api.model
    def get_balance_date_value(self):
        balance_date = date.today()
        return {'balance_date': balance_date}
    ...
    def bank_balance(self, *post):
        company_id = self.get_current_company_value()
        balance_date = date.today()
        if len(post) > 1 and post[1] and post[1]['balance_date']:
            balance_date = datetime.strptime(post[1]['balance_date'], '%Y-%m-%d')
            balance_date = balance_date.strftime(DEFAULT_SERVER_DATE_FORMAT)
        ...
        query = ''' select account_account.name as name, sum(balance) as balance,
                        ...
                        AND account_move_line.company_id in ''' + str(tuple(company_id)) + ''' 
                        AND %s AND account_move_line.date <= %%s
                        group by account_account.name
                                               
                        ''' % (states_arg,)
        self._cr.execute(query, (balance_date,))
